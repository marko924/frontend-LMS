  ...
  proveriStatusIUpis(): void {
    this.sngService.getAllWithoutPagination().subscribe((listaSng: any[]) => {
      ...
      if (mojSng) {
        ...
        this.godinaStudijaId = mojSng.godinaStudijaId || mojSng.godinaStudija?.id;
        this.isUpisan = true;
      ...
      this.predmetService.getAllWithoutPagination().subscribe((predmeti: any[]) => {
        this.nastavnikService.getAllWithoutPagination().subscribe((nastavnici: any[]) => {
          this.pohadjanjeService.getAllWithoutPagination().subscribe((pohadjanja: any[]) => {
            ...
            const predmetiNaGodini = predmeti.filter(p =>
              Number(p.godinaStudijaId) === Number(this.godinaStudijaId)
            );
            const idsPredmetaNaGodini = predmetiNaGodini.map(p => Number(p.id));
 
            const realizacijeZaGodinu = realizacije.filter(r =>
              idsPredmetaNaGodini.includes(Number(r.predmetId))
            );
            ...
            this.predmetiZaPracenje = realizacijeZaGodinu
